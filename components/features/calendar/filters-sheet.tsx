...
import type { ActivityCategory as Category, Venue } from "@/lib/types"
import { SlidersHorizontal, FilterX } from "lucide-react"

export interface CalendarFiltersValues {
  dateFrom?: string
  dateTo?: string
  venueId: string
  categoryId: string
...
interface CalendarFiltersSheetProps {
  ...
  values: CalendarFiltersValues
  onApply: (v: CalendarFiltersValues) => void
  venues: Venue[]
  categories: Category[]
  activeCount: number
}

export function CalendarFiltersSheet({ open, onOpenChange, values, onApply, venues, categories, activeCount }: CalendarFiltersSheetProps) {
  const [draft, setDraft] = React.useState<CalendarFiltersValues>(values)
  React.useEffect(() => {
      ...
      >
        ...
        <div className="flex justify-between items-center mt-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setDraft({ dateFrom: "", dateTo: "", venueId: "all", categoryId: "all" })}
          >
